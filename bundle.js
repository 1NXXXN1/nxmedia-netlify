// FULL bundle.js — Variant B with autoSelect alias
// KinoHub CLONE level: Kinobox direct, KP via /kp proxy, robust normalizers, smartSearch, popular, getFullMovie, player resolver, safe guards
...
// ---------- Kinopoisk proxy via /kp (server-side) ----------
...
class PlayerController {
  ...
  async auto(){ const best = await resolveBestPlayer(this.players); if(!best){ this.emit('error','No working iframe'); return; } const idx = this.players.findIndex(p=>p===best); this.load(idx); }
  autoSelect(){ return this.auto(); } // alias for compatibility
...
// ---------- exports for UI ----------
window.kbPopular = async function(page=1){ try{ return await kbPopular(page); }catch(e){ console.warn("kbPopular err", e && e.message); return []; } };
